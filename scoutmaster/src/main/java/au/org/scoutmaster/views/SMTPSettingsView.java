...
@iFeature(permissions =
	{ @iPermission(action = Action.ACCESS, roles = { eSecurityRole.TECH_SUPPORT})
	, @iPermission(action = Action.DELETE, roles = { eSecurityRole.TECH_SUPPORT})
	, @iPermission(action = Action.EDIT, roles = { eSecurityRole.TECH_SUPPORT})
	, @iPermission(action = Action.NEW, roles = { eSecurityRole.TECH_SUPPORT})
	} )
/** @formatter:on **/
...
public class SMTPSettingsView extends CustomComponent implements View, ValueChangeListener, ClickListener
{
	...
	public SMTPSettingsView()
	{
		...
		// Add both to a panel
		final VerticalLayout fields = new VerticalLayout(this.smtpFQDN, this.authRequired, this.username, this.password,
				this.fromEmailAddress, this.saveButton);
